...
export default function LLMDialog() {
  ...
  const send = (e) => {
    e?.preventDefault();
    if (!text.trim()) return;
    setMsgs((m) => [...m, { role: "user", text }, { role: "assistant", text: "…stub reply…" }]);
    setText("");
  };
  ...
  return (
    <div className="h-full border rounded flex flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-3 py-2 space-y-2 text-sm">
        {msgs.map((m, i) => (
          <div key={i} className={m.role === "user" ? "text-right" : ""}>
            <span className="inline-block border rounded px-2 py-1">
              <strong className="mr-1">{m.role === "user" ? "You:" : "LLM:"}</strong>{m.text}
            </span>
          </div>
        ))}
      </div>
